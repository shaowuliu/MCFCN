...
with tf.device('/gpu:2'):     
    ...
    # 下面就是进行测试的代码，写到一半发现，我的模型是三分类的，但是数据库确实二分的
    # 而且两万张切片的数量还是不多的。
    import numpy as np
    alls = np.zeros(shape=(28580,256,256,1))
    truth = np.zeros(shape=(28580,2))
    title ='Slice-level-labels.npy'  # 读取的文件名
    excel = np.load(title)
    number = 0  #所以切片的顺序 #8048病变切片 28580所有
    number2 = 0 #所有病人的顺序

    for i in range(1,55):
        x = np.load('../lung/covid/covid'+str(i)+'.npy')
        for qiepian in range(0,x.shape[0]):
            alls[number] = x[qiepian]
            if excel[number2,qiepian] == 1:
                truth[number,1] = 1
            else: 
                truth[number,0] = 1
            number = number +1
    ...
    i = 0
    for i in range(1,61):
        x = np.load('../lung/cap/cap'+str(i)+'.npy')
        for qiepian in range(0,x.shape[0]):
            alls[number] = x[qiepian]
            truth[number,0] = 1
            number = number +1
    ...
    i = 0
    for i in range(1,77):
        x = np.load('../lung/normal/normal'+str(i)+'.npy')
        for qiepian in range(0,x.shape[0]):
            alls[number] = x[qiepian]
            truth[number,0] = 1
            number = number +1
