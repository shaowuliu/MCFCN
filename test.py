...
with tf.device('/gpu:0'):     
    ...
    for file in files:
		...
		#normal_img2 = normalize_image(normal_img2)
                normal_img2,normal_img2_512 = sel(covid_path+"/"+file)
                #all_slices[slices_index,:,:,0] = normal_img2
                normal_img2 = np.expand_dims(normal_img2,2)
                normal_img2 = np.expand_dims(normal_img2,0)
                normal_img2_512 = np.expand_dims(normal_img2_512,2)
                normal_img2_512 = np.expand_dims(normal_img2_512,0)
                print('tstest',normal_img2.shape)
                
                result = model.predict(normal_img2,normal_img2_512)
                print(r'adfadf',result)
                if np.argmax(result) != 2:
                    False_res+=1
                    False_name.append(covid_path+"/"+file)
                # all_index[slices_index,2] = 1
                    with open('12.22.txt','a',encoding='utf-8') as f:
                        f.write('{} {}\n'.format(str(False_res),str(covid_path+"/"+file)))
                print(result,file)
                slices_index+=1
    ...
    for file in files:
            ...
            if np.argmax(result) != 0:
                False_res+=1
                False_name.append(normal_path+"/"+file)
                with open('12.22.txt','a',encoding='utf-8') as f:
                    f.write('{} {}\n'.format(str(False_res),str(normal_path+"/"+file)))
    ...
    for file in files:
            ...
            if np.argmax(result) != 1:
                False_res+=1
                False_name.append(cap_path+"/"+file)
                with open('12.22.txt','a',encoding='utf-8') as f:
                    f.write('{} {}\n'.format(str(False_res),str(cap_path+"/"+file)))           
            #all_index[slices_index,1] = 1 
            print(result,file)
            slices_index+=1	

    print(False_name,False_res)
    print(False_res)
    plt.imshow(np.squeeze(normal_img2))
    plt.show()
